...
# Global variable to store moved folders for the 'archives' category
moved_archives = []
    ...
    try:
        ...
        if zipfile.is_zipfile(archive_path):
            ...
            if os.path.exists(extracted_folder):
                ...
                # Вывод на экран названия архива в строке "archives:"
                print(f'{category}: {archive_name}')

                # Запись в файл
                with open(os.path.join(category_path, "results.txt"), 'a') as file:
                    file.write(f"{category}: {archive_name}\n")
...
def list_archives_folders(root_folder, category):
    category_path = os.path.join(root_folder, category)
    try:
        ...
        for category in ['images', 'video', 'documents', 'audio', 'programm', 'archives', 'extensions']:
            ...
            try:
                ...
                if category == 'archives':
                    # Write moved folders for 'archives' category
                    moved_archives_info = [move_and_rename(os.path.join(category_path, archive), category, root_folder) for archive in files]
                    file.write(f"{category}: {', '.join(moved_archives_info)}\n")
                else:
                    file.write(f"{category}: {', '.join(files)}\n")
            except FileNotFoundError:
                print(f"Warning: Category {category} not found.")

        # Вывод названий всех папок в категории "archives"
        archives_folders = list_archives_folders(root_folder, 'archives')
        print(f"\nList of folders in 'archives': {', '.join(archives_folders)}\n")
        file.write(f"\nList of folders in 'archives': {', '.join(archives_folders)}\n")
...
if __name__ == "__main__":
    ...
    for category in ['images', 'video', 'documents', 'audio', 'programm', 'extensions']:
        category_path = os.path.join(folder_path, category)
        try:
            files = list_files_in_category(category_path)
    ...
    # Вывод названий всех папок в категории "archives" на экран
    print("\narchives:")
    archives_folders = list_archives_folders(folder_path, 'archives')
    for folder in archives_folders:
        print(folder)
